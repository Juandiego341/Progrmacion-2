...
class PersonaJuridica extends Persona {
    ...
    calcularDescuento(costoPrueba) {
        let descuento = super.calcularDescuento(costoPrueba); // Descuento base de Persona
        if (this._regimen === 'contributivo' && this._ingreso > 3000) {
            descuento += costoPrueba * 0.05; // Descuento adicional para ingresos altos
        }
        return descuento;
        ...
        for (let laboratorio of this._laboratorios) {
            for (let prueba of laboratorio._pruebas) {
                ...
                prueba._personas.forEach(persona => {
                    ingresosPorRegimen[persona._regimen] += prueba._costo - persona.calcularDescuento(prueba._costo);
                });
